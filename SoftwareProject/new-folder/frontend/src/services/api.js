// services/api.js - IMPROVED VERSION WITH BETTER ERROR HANDLING
...
export const api = {
  async login(emailOrUsername, password) {
    try {
      ...
      console.log('Request body:', { ...requestBody, password: '***' }); // Log without exposing password
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          throw new Error(data.error || data.detail || 'Bad request. Please check your input.');
  ...
  async signup(username, email, password, password2, firstName = '', lastName = '') {
    try {
      ...
      console.log('Signup data:', { username, email, first_name: firstName, last_name: lastName });
